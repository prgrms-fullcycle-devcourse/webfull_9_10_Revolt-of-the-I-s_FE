...
interface DashboardProps {
  ...
  updateTicketStatus: (taskId: number, actionType: 'accept' | 'submit' | 'confirm' | 'reject') => Promise<{ ok: boolean } | undefined>;
...
export const Dashboard = ({ 
  ...
  // member 데이터 가공
  const membersWithUuid = activeTeam?.members.map((m: any) => ({
  ...
  const handleCreateTicket = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const finalTicket = {
        ...
        worker: membersWithUuid.find(m => m.uuid === selectedWorkerUuid)?.name || "담당자",
  ...
  const CreateTicketModal = (
    <Modal isOpen={activeModal === 'create'} onClose={closeCreateModal} title="새로운 업무 요청">
      <form onSubmit={handleCreateTicket} className="space-y-6">
        <div className="space-y-1.5">
          <label className="text-[11px] font-bold text-slate-400 ml-1 uppercase">업무 타이틀</label>
          <input name="title" required className="w-full px-6 py-4 bg-slate-50 rounded-2xl outline-none font-bold" placeholder="업무 핵심 주제" />
        </div>
        <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-400 ml-1 uppercase">담당자 선택</label>
      ...
      <div className="h-full w-full">
        ...
        {activeTeam.tickets.length === 0 ? (
          <div className="h-full w-full flex flex-col items-center justify-center p-8">
            ...
            <button onClick={() => setActiveModal('create')} className="bg-blue-600 text-white px-30 py-5 rounded-4xl font-black text-xl">
              새 요청
            </button>
          </div>
        ) : (
          <div className="flex gap-6 h-full min-w-300">
            {STATUS_TYPES.map((status) => {
              const filteredTickets = activeTeam.tickets.filter((t) => 
                String(t.status).toLowerCase() === String(status.id).toLowerCase()
              );
              return (
                <KanbanColumn
                  key={status.id}
